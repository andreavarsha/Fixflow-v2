...
import { chatJson, type LlmImage, translateText } from "./llm";
import {
  JOB_CATEGORIES,
  normalizeCategory,
  type JobUrgency,
...
export const classifyIssue = internalAction({
  ...
  handler: async (ctx, { jobId, description, photoId }) => {
    try {
      ...
      // Translate English summary to Sinhala and Tamil
      const summary_si = await translateText(classified.summary, "si");
      const summary_ta = await translateText(classified.summary, "ta");
      ...
      await ctx.runMutation(internal.jobs.applyClassification, {
        ...
        aiSummary_si: summary_si,
        aiSummary_ta: summary_ta,
      });
    } catch (error) {
...
export const getTimeline = query({
  ...
  handler: async (ctx, { jobId }) => {
    ...
    const quotedQuotes = quoteRequests.filter(
      (q) => q.status === "quoted" || q.status === "accepted",
    );
    const numQuotes = quotedQuotes.length;
    const hasInvites = quoteRequests.length > 0;
    ...
    const steps = [
      { id: "classifying", label: "Analyzing issue", at: job._creationTime },
      {
        id: "finding",
        label: "Finding pros",
        at: job.status !== "classifying" ? job._creationTime : undefined,
      },
      {
        id: "quotes",
        label: numQuotes > 0 ? `Quotes in (${numQuotes})` : "Quotes received",
        at: hasInvites ? quoteRequests[0]._creationTime : undefined,
      },
      {
        id: "accepted",
        label: "Accepted",
