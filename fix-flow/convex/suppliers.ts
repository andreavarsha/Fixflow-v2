...
export async function getSupplierStats(
  ctx: QueryCtx,
  supplierId: Id<"users">,
): Promise<{ completedJobs: number; avgResponseMinutes?: number }> {
  const supplierQuotes = await ctx.db
    .query("quoteRequests")
  ...
  for (const q of supplierQuotes) {
    if (q.status === "accepted") {
      const job = await ctx.db.get(q.jobId);
      if (job && job.status === "completed") {
        completedJobs++;
      }
    }
    if (q.quotedAt !== undefined) {
      const responseMs = q.quotedAt - q._creationTime;
  ...
  return { completedJobs, avgResponseMinutes };
}
...
async function findNearbySuppliers(
  ...
  do {
    ...
    for (const hit of page.results) {
      ...
      const stats = await getSupplierStats(ctx, supplier._id);
